...
%         measure: what data are we looking for. the current options are
%           only measures I needed for my honours project that were not
%           attainable in a more direct way. In the future, if we hope to
%           use something like this, it should probably be expanded.
%           current measure options:
%                     'cond': condition, only relevant for feedback2 and
%                     feedback3 (9 sec or 1 sec feedback condition). one
%                     row per participant
%                     'cp': subject's CP (first of 24 in a row correct
%                     trials) aka point at which we can say they learned.
%                     one row per participant.
%                     'rt2': phase 2 duration aka how long stimulus was
%                     onscreen before participant made a category
%                     selection. one row per trial per participant.
%                     'rt4': phase 4 duration aka how long feedback was
%                     onscreen before participant moved on to next trial.
%                     one row per trial per participant. 
%                     'accuracy': 1 if participant answered correctly, 0 if
%                     they answered incorrectly. one row per trial per
%                     participant. 
%                     'durp2': mean duration of fixations to features
%                     (funcRelevance = 1, 2, or 3) during phase 2. one row
%                     per trial per participant
%                     'durp4': mean duration of fixations to features
%                     (funcRelevance = 1, 2, or 3) during phase 4. one row
%                     per trial per participant
%                     'fc2': number of fixations to features (funcRelevance
%                     = 1, 2, or 3) during phase 2. one row per trial per
%                     participant
%                     'fc4': number of fixations to features (funcRelevance
%                     = 1, 2, or 3) during phase 4. one row per trial per
%                     participant
%                     'irrel2': number of fixations to irrrelevant feature 
%                     (funcRelevance = 3) during phase 2. one row per 
%                     trial per participant
%                     'irrel4': number of fixations to irrrelevant feature 
%                     (funcRelevance = 3) during phase 4. one row per 
%                     trial per participant
%                     'opt': gaze optimization during phase 2. one row per
%                     trial per participant.
%                     'opt4': gaze optimization during phase 4. one row per
%                     trial per participant.
%                     'p4button': time fixating  feedback buttons
%                     (funcRelevance = 4, 5, 6, or 7) during phase 4. one
%                     row per trial per participant.
%                     'p4feature': time fixating stimulus features
%                     (funcRelevance = 1, 2, or 3) during phase 4. one row
%                     per trial per participant. this may be exactly the
%                     same as fc4 (oops LOL)
...
switch measure
    ...
    case 'rt2'      % pull straight from trial lvl
        ...
        for i = 1:length(subjects)
           current = subjects(i); 
           % luckily, Phase2RT is already a variable in trial level tables,
    ...
    case 'rt4'  % pull straight from trial lvl
        ...
        for i = 1:length(subjects)
           current = subjects(i); 
           % same as for p2, Phase4RT is a variable in the tables
    ...
    case 'accuracy'     % pull straight from trial lvl
        ...
        for i = 1:length(subjects)
           current = subjects(i); 
           % similar to p2, p4 rt, accuracy is just a column in trial level
           % tables
    ...
    case 'durp2'   % fixation time ON FEATURES (average)
        ...
        % stimulus features for each trial
        ...
        for i = 1:length(subjects)
           ...
           for j = 1:maxTrial
               % for every trial for the current subject, find all fixations with funcRelevance = 1, 2, or 3
    ...
    case 'fc2'      % same as above, but count using length() instead of taking a mean
        % here we are counting the number of fixations to features during
        % p2 of each trial
        ...
        for i = 1:length(subjects)
           ...
           for j = 1:maxTrial
               ...
               p2 = thisTrial(thisTrial.TrialPhase == 2, :);
                ...
                % p2 will be a vector of all the subject's fixations (that
                % we care about, aka to features) during p2 in the current
                % trial. the length of this vector is the number of
                % fixations.
               p2 = p2.Duration(p2.funcRelevance > 0 & p2.funcRelevance < 4);
               if isempty(p2)
    ...
    case 'irrel2'   
        % time spent on irrelevant features in p2
           ...
           for j = 1:maxTrial
               ...
               if isempty(p2)
                   optimization = NaN;  % no p2 value means we can't calculate opt
                      ...
                      % we divide relevant by 2 here because are two
                      % relevant features:1 irrelevant feature on every
                      % trial. in optimization, we are looking for a
                      ...
                      % features). the way to make this work is to divide
                      % relevant by two (write it out and check for
